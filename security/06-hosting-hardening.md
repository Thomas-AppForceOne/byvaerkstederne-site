...
**Note:** These headers are set in Grav's `.htaccess` at the root of `config/www/`:
...
This configuration is present in the Grav default `.htaccess` and has been verified to be in the deployed production `.htaccess`.
...
| 5. Security response headers | ✅ Confirmed | — | No action required |
