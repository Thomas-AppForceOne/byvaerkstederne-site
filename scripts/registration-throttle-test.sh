#!/usr/bin/env bash
# registration-throttle-test.sh — one-command LOCAL verification of the
...
#
# Usage: scripts/registration-throttle-test.sh [attempts]   (default 6)
...
echo ""
OUT="$(mktemp)"
"$SCRIPT_DIR/test-registration-throttle.sh" "http://localhost:$PORT" "$ATTEMPTS" | tee "$OUT" || true
echo ""
