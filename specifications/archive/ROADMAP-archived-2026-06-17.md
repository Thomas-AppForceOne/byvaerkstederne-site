> **RETIRED 2026-06-17.** This is the original implementation-order tracker for the
> deploy/data-lifecycle, member-auth, and frontend arc. It was a status tracker that
> mixed implemented and planned steps, which the current roadmap rules disallow.
> Superseded by the lean, forward-only [`specifications/ROADMAP.md`](../ROADMAP.md).
> Preserved verbatim below; do not edit (archived documents are immutable).
